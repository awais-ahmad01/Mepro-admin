...
export default function VIPPricing() {
  const [plans, setPlans] = useState<VIPPricingPlan[]>(defaultPlans);
  const [openDialog, setOpenDialog] = useState(false);
  const [editingPlan, setEditingPlan] = useState<VIPPricingPlan | null>(null);
  ...
  const handleEdit = (plan: VIPPricingPlan) => {
    setEditingPlan(plan);
    setFormData({
      type: plan.type,
  ...
  return (
    <Box sx={{ p: 3, minHeight: "100vh", background: "#F7F8FA" }}>
      ...
      >
        {plans.map((plan) => (
          <Box key={plan.id} sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <Paper
              sx={{
                p: 3,
                borderRadius: 3,
            ...
            >
              <Typography variant="h5" fontWeight={600} mb={1} display="flex" alignItems="center" justifyContent="center" gap={1}>
                {plan.type} <LockIcon sx={{ fontSize: 22, ml: 0.5 }} />
              </Typography>
              ...
              <Box display="flex" alignItems="center" justifyContent="center" mb={4}>
                ...
                <DiamondIcon sx={{ color: "#FF4D7D", ml: 1, fontSize: 32 }} />
              </Box>
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          {editingPlan ? "Edit VIP Pricing Plan" : "Create New VIP Pricing Plan"}
        </DialogTitle>
        <DialogContent>
          <Box mt={2}>
            <Typography mb={1} fontWeight={500}>Plan Type</Typography>
            <TextField
              fullWidth
              value={formData.type}
              onChange={(e) => setFormData(prev => ({ ...prev, type: e.target.value }))}
              placeholder="e.g. VIP Basic, Silver, Gold, etc."
              sx={{ mb: 2 }}
            />

            <Typography mb={1} fontWeight={500}>Price</Typography>
            <TextField
              fullWidth
            ...
            <Typography mb={1} fontWeight={500}>Period</Typography>
            <TextField
              fullWidth
              value={formData.period}
              onChange={(e) => setFormData(prev => ({ ...prev, period: e.target.value }))}
              placeholder="e.g. Monthly Charge, Quarter Charges, Per Year"
              sx={{ mb: 3 }}
            />

            <Typography mb={1} fontWeight={500}>Features</Typography>
            {formData.features.map((feature, index) => (
              <Box key={index} display="flex" gap={1} mb={2}>
                ...
                <Button
                  ...
                  sx={{ color: "#D92D20", minWidth: 0, p: 1 }}
                >
                  <DeleteIcon />
                </Button>
              </Box>
